...
<div class="flex h-full w-full items-center justify-center">
	<div class="w-full flex-col">
		...
		<div class="m-4">
			...
			<a class="text-sm inline-block min-h-[24px]" href="javascript:void(0);">{messages['login']['forgot_password']}</a>
		</div>

		<div class="m-4">
			<!-- disbling the warning until we have valid link for this -->
			<!-- eslint-disable-next-line svelte/valid-compile -->
			<a class="text-sm inline-block min-h-[24px]" href="javascript:void(0);">{messages['login']['create_account']}</a>
		</div>
	</div>
</div>
