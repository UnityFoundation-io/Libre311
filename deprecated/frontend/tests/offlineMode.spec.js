...
test("report new issue in offline mode", async ({ page, context }) => {
  ...
  await expect(
    page.getByRole("button", { name: "Report a New Issue" })
  ).toBeVisible();
  await context.setOffline(true);
  await page.getByRole("button", { name: "Report a New Issue" }).click();
  ...
  // Review information
  await expect(
    page.getByText("Issue Location: 12140 Woodcrest Executive Drive")
  ).toBeVisible();
  ...
  await expect(
    page.getByText("Thank You! The issue will be reported when you go online.")
  ).toBeVisible();
